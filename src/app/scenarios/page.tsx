...
export default function ScenariosPage() {
  ...
  const [ownerId, setOwnerId] = useState("owner_a");
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      setError(null);
      try {
        const res = await fetch("/api/scenario/public");
        const json = (await res.json().catch(() => ({}))) as PublicResponse;
        if (!res.ok) {
          throw new Error("Failed to load public scenarios");
        }
        if (!cancelled) {
        ...
        if (!cancelled) {
          setError(e?.message ?? "Failed to load public scenarios");
        }
      } finally {
        if (!cancelled) {
          setLoading(false);
